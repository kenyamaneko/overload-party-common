...
> **注:** スクリプト（テキスト）はサーバーAPI経由、スクリプト内で参照される画像・音声アセットは Cloudflare CDN（GCS 公開バケット）から直接取得する2層構造。詳細は [ARCHITECTURE.md §14.4](ARCHITECTURE.md) を参照。
...
1. **スクリプトファイルを作成しサーバーに配置**
   ...
   > TODO: サーバー実装確定後に具体的な配置先・配置方法を記載する
