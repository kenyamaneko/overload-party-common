...
- **メトリクス**: Cloud Monitoring。Cloud Run が標準で出す要求数・応答時間・ログ件数を使う
- **ログ**: Cloud Logging。構造化ログ（JSON）を使用し、`gameID` / `playerID` でフィルタ可能にする
- **アラート**: 5xx 応答、応答時間、ERROR ログ、ジョブの失敗に対してアラートポリシーを設定（[ADR-063](../adr/063-cloud-run-monitoring-and-alerting.md)）
