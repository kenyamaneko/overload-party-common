...
1. **ラムネ + エリス**: スループット 700（エリス Trigger）で RC 0 攻撃。Elastic で最大 1,200。無料で殴れる主力に
...
5. **えくぼ C系 medium + SD Front + SD Gateway** = スループット 2,100+200+200 = 2,500。1体で Revenue 2,500
...
3. **空即蒼色 R系 medium + マルチ AZ + 天気使い Backup**: 可用性 5,400+500 = 5,900。破壊 →可用性 2,950 で復活（1回限り）
...
1. **コンダクティス Always Free + 調律部 DevOps**: large まで合計 Dep 300。R系 large 可用性 = 8,100
2. **アピエッタ ×2 + カンタータ ×3**: 無料フロントエンド2体 + 安定フロントエンド3体。フロントエンド5種で途切れにくい
3. **音の魔術師 ×3 + リアル・アンサンブル・クラスター ×3**: Yield 1,500→2,100/T（Elastic で自動上昇）。各可用性 1,900。調律部 License で全回復可能
4. **調律部 Failback ×2 + 音の魔術師 ×3**: DB 破壊 → 手札音の魔術師が即復帰。DB パイプラインが途切れない

### 調律部特有の課題

CE/CA が実質使用不可。ドローは寺リフォーム + 通常ドロー（1枚/T）のみ。アピエッタ ×2 + カンタータ ×3 + プロジェクトマネージャー ×3 で手札の薄さを補い、圧倒的 Revenue 差（Yield 1,500→2,100/T、Elastic 最大時 3,000/T 超）で Budget 勝ちを狙う。音の魔術師は E のため Scale Up 不可だが、Revenue が回れば自動で Yield 上限に達する。
