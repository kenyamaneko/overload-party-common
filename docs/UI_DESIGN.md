...
陣営（ユニット）選択 — ユニ君が4陣営を紹介
    │
    ├── SD: 物流の熱量で世界を繋ぎ止めようとする者
    ├── 天気使い: 天候の障壁で平穏を守り抜こうとする者
    ├── しゅがーLab: 創造の遊び心で絶望を塗り替えようとする者
...
#### 10.1 アタッチメント表示（下スタック + バッジ）

各リソースは最大2枚のアタッチメントスロットを持つ。フィールド上ではリソースカードの下にアタッチメントスロットが重なる形で表示する。
...
- **バッジ:** 右上に装備数を表示（0/2 = 非表示、1/2 = 青、2/2 = オレンジ）
- **下スロット:** メインカードの下にアタッチメントが少しだけはみ出す
  - 装備済み: 陣営カラーの枠にアイコン + カード名
  - 空き: 破線のゴースト枠（ATTACH SLOT）
  - 全スロット使用時: ゴースト枠が消え「もう付けられない」ことが直感的にわかる
- **ストライプライン:** 装備済みスロットにはカード陣営カラーのアクセントライン

**アタッチメント操作:**

1. 手札の Attachment カードをタップ → 自フィールドのアタッチ可能なリソースが赤く光る
2. 光っているリソースをタップ → アタッチ実行（コスト無料）
3. アタッチ不可（スロット満杯）のリソースは光らない
4. 背景タップで選択解除
...
- フィルター: 陣営（SD / 天気使い / しゅがーLab / 調律部 / Neutral）、タイプ、コスト
- ソート: カード番号順 / コスト順 / 陣営順
...
| セット名 | 内容 | 状態 |
|---------|------|------|
| SDセット | SD陣営の全カード | 購入済み / 購入ボタン |
| 天気使いセット | 天気使い陣営の全カード | 購入済み / 購入ボタン |
| しゅがーLabセット | しゅがーLab陣営の全カード | 購入済み / 購入ボタン |
...
**スタンプタブ:**
- 陣営スタンプセット（SD / 天気使い / しゅがーLab / 調律部: 各480円）
