...
## Amendment: 2026-07-17 artifact 保持の取り消し

ADR-055 がテスト観点カタログを common への集約から各リポの GitHub Pages への個別公開に設計変更したため、リポをまたいだ artifact 取得が不要になった。カタログ生成は各リポの CI 内でテスト結果ファイルをその場で読むだけで完結するため、上記 Amendment (2026-07-15) の artifact 保持有効化を取り消す。結果ファイルを artifact として保持しないという本文の結論に戻る。
