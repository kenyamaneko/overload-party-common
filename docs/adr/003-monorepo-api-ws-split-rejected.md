...
**Superseded by:** ADR-002 (battle-server-csharp-separation)
**Reconstructed:** 2026-04-22

---

> **注記**: この ADR は当時実際には起票されなかった設計判断を、**後日、残存するメモと git 履歴から推測して再構築**したものです。オリジナルの記録は存在しません。推測の根拠は [「推測の根拠」](#推測の根拠) に記載。
...
ADR-002（battle-server-csharp-separation）の採用により、分割軸が **REST/WS 軸** ではなく **責務軸（Gateway / Battle）** に変更された：
