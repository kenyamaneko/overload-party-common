...
カテゴリの振り分けは、どのセクションにも入らない由来があればカタログの生成を失敗させる。既定のカテゴリへ落とすと分類されないまま紛れ込むため、テストを持つパッケージが増えたときに振り分け設定の更新を強制する。複数のカテゴリに当てはまる由来は、より具体的な指定を持つカテゴリに入れる。上位のまとまりを一方のカテゴリに置き、その一部だけを他方へ分けられるようにするため。
