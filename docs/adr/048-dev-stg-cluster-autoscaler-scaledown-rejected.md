...
ノードスケールダウンを GKE Cluster Autoscaler へ寄せ、LB は常時起動を許容する案は、掲げた 2 つの便益 (コスト削減・運用簡素化) をいずれも達成できないため採用しない。dev/stg のコスト削減は ADR-018 の node-resize + LB / DNS teardown スクリプト方式を継続する。ノードスケールダウンの autoscaler 化は、Pod scale-0 の仕組みが別の理由で導入される時点で改めて再検討する。
