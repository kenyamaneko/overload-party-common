...
- Related: [ADR-012](012-matchmaking-pubsub.md) (Pub/Sub 型の送信側所有), [ADR-015](015-package-split.md) (パッケージ分割と SSoT 分散), [ADR-016](016-repository-testing-testcontainers.md) (リポジトリ層テスト戦略), [ADR-017](017-game-config-firestore.md) (Firestore), [ADR-011](011-repository-split.md) (repository split)
...
また [ADR-016](016-repository-testing-testcontainers.md) は **Cloud SQL と同じ DB エンジンで検証する** ことを決めたが、「本物の Cloud SQL / 本物の Firestore / 本物の GCP Pub/Sub」に対する検証は CI 経路に存在しない。emulator / Testcontainers と本番の乖離（Firestore インデックス要求、Cloud SQL 接続プール設定、Pub/Sub Exactly-Once 配信の挙動等）は手動動作確認に依存している。
...
### B. 各サービスが相手サービスの fake を独自に手書きし続ける
...
### C. Contract Testing (Pact 等) の全面導入
...
### D. テストピラミッド + 送信側パッケージへの fake 同梱 + nightly cloud integration（採用）

[ADR-015](015-package-split.md) の送信側所有原則をテストダブルに拡張する。送信側サービスが「型 + fake/test double」を 1 つのパッケージで配布し、consumer 側はそれを import するだけ。emulator / fake と本番の乖離は nightly の cloud-verified integration test で検知する。
...
| 4 | **クラウド検証テスト** (cloud-verified) | emulator / fake と本番実装の乖離検出 | 本物の Cloud SQL / Firestore / GCP Pub/Sub (stg) | `cloud_integration` | nightly |
...
- **アダプターテスト**: プロジェクト共通のクリーンアーキテクチャ用語（CLAUDE.md 参照）である "adapter" に揃えた呼称。DB 境界 (repository)、Pub/Sub 境界 (pubsub adapter)、外部 HTTP クライアント境界のいずれも adapter 層に該当する。[ADR-016](016-repository-testing-testcontainers.md) の「リポジトリ層テスト」は本名称のうち **DB 境界を扱うサブセット** として位置づけ直される（ADR-016 の方針自体は変更しない）
...
- **GCP Pub/Sub**: Exactly-Once Delivery の実挙動（[ADR-012](012-matchmaking-pubsub.md) が前提にする保証）、dead letter queue への遷移、メッセージ重複の実測
...
- **Contract Testing (Pact 等)** は本 ADR では導入しない。nightly cloud_integration で乖離検知を行い、実害が顕在化したら再検討する
...
- [ADR-015](015-package-split.md) の「送信側所有」原則がテストダブル層にも一貫して適用され、契約変更時の consumer 側追従が import 更新だけで済む
- テストの所有権が各サービス repo に閉じるため「落ちたテストを誰が直すか」が自明になる
- **"アダプターテスト" と "サービス間結合テスト" を語彙として分離**したことで、レビュー時・新サービス立ち上げ時に「このテストは何を証明しているか」が会話の前提として揃う（従来の「統合テスト」「結合テスト」の曖昧さを解消）
- nightly cloud integration により emulator / 本番の乖離を継続的に検知できる（Firestore インデックス要求、Pub/Sub Exactly-Once 挙動など）
- 新サービスを追加するとき、既存サービスの fake パッケージを import するだけで subscribe テストが書ける（共通テストユーティリティの再発明が不要）
- build tag / CI ジョブ分離により unit ジョブが Docker 非依存になり、軽量化する
...
- [ADR-016](016-repository-testing-testcontainers.md) のリポジトリ層テスト戦略（Testcontainers、TRUNCATE リセット、postgres:16-alpine）はそのまま継続。本 ADR はその戦略を **"アダプターテストの DB 境界サブセット"** として上位概念に位置づけ直すのみで、テストコード自体に変更は生じない
- [ADR-012](012-matchmaking-pubsub.md) の Exactly-Once 通知・[ADR-017](017-game-config-firestore.md) の Firestore 運用は本 ADR で変更しない

## Migration Plan

First pilot (`overload-party-shop`) を以下の順で進める。

1. **build tag の全面付与**: shop の既存 integration テストに `//go:build integration` を付ける。CI を unit / integration の 2 ジョブに分離
2. **`apishopfake` の実装**: [packages/api-shop](../../../overload-party-shop/packages/api-shop/) に broker + publisher fake + subscriber fake を追加。shop 自身の publisher テストを fake ベースに移行
3. **nightly cloud_integration ワークフロー**: shop repo に `.github/workflows/nightly-cloud-integration.yaml` を追加。Cloud SQL migration smoke + 実 Pub/Sub Exactly-Once smoke を実装
4. **CLAUDE.md への反映**: shop と common の CLAUDE.md に本 ADR への参照を追加（ただしこの反映は人間による明示指示で行う）

Second pilot (`overload-party-account`) は first pilot 完了後に：

5. shop の `apishopfake` を import した subscriber テストを account に実装
6. `player-onboarded` 等 account 側が consume する他トピックについても同様の fake を対応する送信側パッケージに追加
7. first pilot で確立したテンプレ（CI ジョブ構成・nightly workflow・tag 運用）を account にコピー展開

Third pilot 以降（gateway / card / matchmaking / scenario / support / news / newsfeed / battle）は、first / second pilot で確立したテンプレを各サービスに適用していく。C# (battle) と Python (newsfeed) は言語固有のテンプレを別途整備する。

## Follow-ups

- **E2E 戦略 ADR**: overload-party-e2e の中身（どのフローを E2E で担うか、どの基盤で動かすか）は別 ADR として切り出す
- **Contract Testing の再検討タイミング**: nightly cloud_integration で乖離が実害として複数回検出された場合、Pact 等の consumer-driven contract test 導入を再検討する
- **CLAUDE.md への反映**: 本 ADR 採用後、各サービス repo の CLAUDE.md に「テストダブルの手書き禁止、送信側パッケージの fake を使う」等の方針を追記する（人間からの明示指示で実施）
