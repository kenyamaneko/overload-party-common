...
- Related: [ADR-017](017-game-config-firestore.md) (game_config の Firestore 移管), [ADR-011](011-repository-split.md) (repository split), [ADR-014](014-db-schema-split-per-service.md) (db schema split per service)

> 本 ADR は [ADR-017](017-game-config-firestore.md) Migration Plan step 2（`ops/firestore-seed/seed_game_config.py` に初期値を number で投入）を部分的に上書きする。SSoT の置き場を ops の Python literal から common の YAML に移す変更で、ADR-017 の主決定（Firestore 採用）と運用原則（運用中は Firestore が SSoT、書き込みは運営オペレーター + ops SA に限定）は維持する。
...
- common/data 配下でゲーム設計値を一望できる（`deck_size` と同じ場所）
- ops が pure runner になり、db-migrate と役割分担が揃う
- 設計値の変更は common の PR 1 本で完結（ops への PR 不要）
- lock file パターンの再利用により、将来的に追加する seed 系（他の Firestore コレクションや external config）も同じ形で拡張可能
...
- Firestore への書き込み権限モデル、fail-fast 原則、エミュレーター構成は ADR-017 のまま
...
- CI で `--fetch` 経路を自動実行する場合は `.github/actions/firestore-seed/` 相当の composite action を用意（ADR-017 `.github/actions/firestore-emulator/` に倣う）。本 ADR 時点では手動実行運用のため未着手
