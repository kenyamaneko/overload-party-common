...
**Status:** Proposed (§決定.1 の subscriber 副作用テーブル「account: `players.display_name` UPDATE」は [ADR-025](025-onboarding-name-via-rest-and-cross-service-http.md) により部分的に supersede)
**Date:** 2026-04-22

> 本 ADR は [ADR-021](021-onboarding-scenario.md) §5 の 2 イベント設計 (`player-onboarded` + `faction-selected`) を部分的に上書きする。scenario の onboarding 完了に伴う publish は `player-onboarded` 1 本に縮退し、shop 起因の faction 取得は新トピック `faction-purchased` として独立する。
>
> **本 ADR の §決定.1 で account subscriber の副作用に挙げた `players.display_name` UPDATE は [ADR-025](025-onboarding-name-via-rest-and-cross-service-http.md) で除外される**。表示名はオンボード内 name 入力ステップで scenario が account の `PUT /internal/v1/players/:playerId/name` を同期 REST で呼んで確定するため、`PlayerOnboardedEvent` payload には `display_name` を載せず、subscriber では表示名の反映を行わない (account 側 subscriber は `player_factions` INSERT + `players.selected_faction` UPDATE のみ)。card / gateway 側の副作用は変更なし。
...
### 2. オンボーディング起因の発火が `PlayerOnboardedEvent` と情報重複している

[ADR-021](021-onboarding-scenario.md) §5 で scenario は onboarding 完了時に以下の 2 イベントを同時 publish する設計とした:
...
### 3. `common/packages/pubsub-events` の役割が過剰
...
`FactionSelectedEvent` を廃止し、業務事実単位で以下の 2 イベントに分解する。

### 1. オンボーディング起因 → `PlayerOnboardedEvent` 単体で完結
...
scenario の outbox は `PlayerOnboardedEvent` 1 行のみ enqueue する。ADR-021 §6.1 の「2 イベント同一トランザクション」は `PlayerOnboardedEvent` 1 本の enqueue に縮退する。

### 2. shop 購入起因 → `FactionPurchasedEvent` を新設
...
### 3. イベント型の配置 (ADR-015 の送信側所有原則に完全準拠)
...
### 4. `common/packages/pubsub-events` の廃止
...
### 案 1: 現状維持 (FactionSelectedEvent を残す)
...
### 案 2: `FactionSelectedEvent` を shop/api-shop に移動 (前回検討した案 β)
...
- 案 1 と同じく `PlayerOnboardedEvent` との冗長性が解消されない

### 案 3: `PlayerOnboardedEvent` に Source を追加して faction-purchased をぶら下げる

単一イベントで 2 業務事実を扱う方向で統合。

却下理由:
- shop 購入時に "onboarded" を名乗るのは意味的におかしい
- 分岐の方向が逆転するだけで本質は案 1 と同じ
...
- **命名と実態の一致**: `PlayerOnboardedEvent` は onboarding 完了、`FactionPurchasedEvent` は購入取引、という業務事実と 1 対 1 対応
- **subscriber 側コードの簡素化**: Source 分岐が全 subscriber から消え、handler が「このイベントだけ」を処理する形にフラット化
- **ADR-021 の 2 イベント atomic 設計が 1 イベントに縮退**: outbox への enqueue が 1 行のみになり、同一トランザクション整合性の配線も単純化
- **common/packages/pubsub-events の完全廃止**: module 管理・Dockerfile secret・go.mod require の運用負債が 5 リポから消える (common リポは game-design-constants / doc-tools のためには残る)
- **ADR-015 / ADR-012 の送信側所有原則に実態が追いつく**: `PremiumUpdatedEvent` (shop 単独) が原則違反のまま common にあったズレを解消
...
- **ADR-021 の supersede**: ADR-021 §5 と §6.1 の「2 イベント同一トランザクション」設計を本 ADR で縮退させる必要がある (ADR-021 本文に note を追記)
- **scenario の event_builder が 1 メソッドに減る**: `BuildFactionSelected` 削除で、scenario 側の outbox 行の種類は 1 種類のみに

### 移行ステップ

以下の順序で実装する。`main` 直コミット運用なので stage 単位で commit / 動作確認する。

1. **ADR-022 採用** (本 ADR を main にマージ)
2. **overload-party-infra (Terraform)**:
   - `faction-selected` topic / subscriptions / IAM を削除
   - `faction-purchased` topic / subscriptions / IAM を新設
   - `player-onboarded` に card / gateway subscription + IAM を追加
   - scenario SA から faction-purchased publisher 権限を付与しない (shop 単独)
3. **overload-party-shop/packages/api-shop**:
   - `FactionPurchasedEvent` / 関連定数を追加
   - `PremiumUpdatedEvent` / 関連定数を追加 (common からの移動)
   - `v0.1.5` タグ push
4. **overload-party-shop** 本体:
   - `event_builder.go` を `BuildFactionPurchased` に rename、Source フィールド削除、import を api-shop に切替
   - publisher の topic map を faction-purchased に差し替え
   - go.mod: pubsub-events require 削除、api-shop の replace を継続
5. **overload-party-scenario**:
   - `event_builder.go` から `BuildFactionSelected` メソッド削除
   - outbox への enqueue を `PlayerOnboardedEvent` 1 本に縮退
   - config.go から `FACTION_SELECTED_TOPIC` env 削除
   - pubsub-events require 削除
   - ADR-021 本文に「ADR-022 により 2 イベント設計から 1 イベントに縮退」の supersede note を追記
6. **overload-party-account**:
   - `faction_selected_subscriber.go` → `faction_purchased_subscriber.go` に rename + Source 分岐削除
   - `player_onboarded_subscriber.go` に faction ownership INSERT + `selected_faction` UPDATE の副作用を追加
   - go.mod に api-shop 追加、pubsub-events 削除
   - Dockerfile に `SHOP_GO_MODULES_FETCH` secret 追加
7. **overload-party-card**:
   - `faction_selected_subscriber.go` → `faction_purchased_subscriber.go` に rename、Neutral 配布ロジックを削除 (faction のカードのみ)
   - `player_onboarded_subscriber.go` 新規 (initial pack = faction + Neutral)
   - config.go に PLAYER_ONBOARDED_SUBSCRIPTION 追加
   - go.mod に api-shop + api-scenario 追加、pubsub-events 削除
   - Dockerfile に必要な secret 追加
8. **overload-party-gateway**:
   - 同上パターン (2 subscriber に分割、WS メッセージ名も 2 種類に)
   - go.mod: api-shop bump + api-scenario 新規追加
9. **overload-party-common**:
   - `packages/pubsub-events/` 削除
   - 関連 publish workflow 削除
10. **overload-party-k8s**:
    - ConfigMap key rename (`faction-selected-*` → `faction-purchased-*`)
    - 各 deployment の env 名変更
    - card / gateway に `PLAYER_ONBOARDED_SUBSCRIPTION` env 追加
    - scenario から `FACTION_SELECTED_TOPIC` env 削除

scenario は ADR-021 の実装が既に main にマージ済みのため、本 ADR で outbox / event_builder の一部を「戻す」形の変更が入る。ADR-021 との関係は本 ADR の supersede note で示す。

---

## 関連 ADR

- **[ADR-012](012-matchmaking-pubsub.md)**: Pub/Sub 設計原則 (送信側型所有・event_id 払出・subscriber 冪等性)。本 ADR の分解と送信側配置は ADR-012 の原則に完全準拠する
- **[ADR-015](015-package-split.md)**: パッケージ分割と SSoT 分散。本 ADR で `common/packages/pubsub-events` 廃止により、event 型は送信サービスの api-<svc> に一本化される (`PlayerOnboardedEvent` → api-scenario、`FactionPurchasedEvent` / `PremiumUpdatedEvent` → api-shop)
- **[ADR-021](021-onboarding-scenario.md)**: オンボーディングシナリオの実装。§5 の 2 イベント設計 (`player-onboarded` + `faction-selected`) を本 ADR で 1 イベント (`player-onboarded` のみ) に縮退する。ADR-021 側に supersede note を追記する
