...
### 言語 / ランナー: TypeScript + Playwright Test
...
### 型のソースは `@kenyamaneko/overload-party-api-gateway` 1 本
...
### ローカル実行環境: e2e 所有の docker-compose
...
- dev/stg を共有環境として使うため、テストデータが残留する。プレフィックス + 手動/定期 cleanup で対応する（cleanup 運用は Phase 2）
