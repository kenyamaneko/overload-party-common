...
DB インスタンスは 1 つのまま維持したうえで、**PostgreSQL スキーマをサービス単位に分割する**。各サービスは自分が所有するスキーマのみに直接アクセスでき、他サービスのデータが必要な場合は所有サービスの REST API 経由で取得する。
...
- **責務境界の明確化**: どのテーブルをどのサービスが所有しているかが DB 権限レベルで強制される。クロスサービスアクセスは物理的に不可能になり、仕様と実装の乖離が起きにくくなる
- **将来の物理分割がコード変更不要**: すでにサービスは自スキーマしか触っていない状態になっているため、将来 DB インスタンスをサービス単位に分けたくなっても、変更は接続先とデータ移行だけで済む。アプリ側のクエリを書き換える必要はない
- **ADR-011 との整合**: リポジトリ分割（アプリ境界）と DB スキーマ分割（データ境界）が揃い、マイクロサービス境界が二重に強制される
- **コスト抑制**: DB インスタンス数は 1 のままなので固定費は据え置ける
...
- **ADR-002**: バトルサーバーの Go → C# 分離。DB レイヤーでも battle の境界を強制する本 ADR の前提
- **ADR-010**: マッチメイキングキューの Upstash Redis 採用。matchmaking が RDB スキーマを必要としない理由
- **ADR-011**: リポジトリ分割。本 ADR はアプリ境界と DB 境界を揃えるためにリポジトリ分割と対応する
- **ADR-012**: マッチメイキングキューの運用方式（Redis 継続）。ADR-010 を補完し、matchmaking が RDB を持たない方針を確定させる

---

## 補遺: 最終確定（2026-04-11）

本 ADR 本体の「スキーマ配置案」には `game_config` / `cosmetic_items` / `factions` 等の横断マスタに関する placeholder を残していたが、その後の対話で**最終確定**した内容を以下に記録する。本体の配置案を読む際は、本補遺を上書き指示として扱うこと。
...
本補遺の確定に伴い、以下のドキュメントは既に更新済みである。

- [docs/architecture/ARCHITECTURE.md §5.1 スキーマ分割とオーナーシップ](../architecture/ARCHITECTURE.md)
- [docs/architecture/DATA_DESIGN.md スキーマ所有権マップ](../architecture/DATA_DESIGN.md)

本 ADR 本体（「スキーマ配置案」節と「`game_config`, `cosmetic_items` 等の横断マスタ」に関する注記）は歴史的経緯として残しているが、**現行の方針は本補遺が SSoT** となる。
