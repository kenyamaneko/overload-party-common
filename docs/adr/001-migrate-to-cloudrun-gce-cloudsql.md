...
- コスト面では有利だが、設計の複雑さがMVPに不釣り合い

### 案2: Cloud Run + Memorystore for Valkey
却下理由:
- Valkey は Pub/Sub + 状態管理を1つのサービスで解決できる
- しかし ~$35/月の追加コストが初期段階には過剰
- WS を単一 GCE にすれば Valkey 自体が不要

### 案3: PostgreSQL LISTEN/NOTIFY
却下理由:
- Cloud Run のエフェメラル特性と根本的に非互換
- LISTEN 接続はインスタンスの常時起動が前提
- Cloud Run のゼロスケールの利点と矛盾

## 結果

- インフラコスト: ~$41-46/月 (70% 以上削減)
- コード変更範囲: DB 層 (Repository 実装追加) + サーバー分割 (cmd/api, cmd/ws)
- 変更なし: ゲームエンジン、サービス層、WebSocket Manager、マッチメイキング
...
- Repository パターンにより、DB 実装の差し替えが容易
- Spanner コードは移行完了後に別 PR で削除予定
