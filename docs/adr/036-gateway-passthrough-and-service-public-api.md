...
### 各サービスが client 公開 API を整備する

各サービス (shop / card / account / scenario / news 等) は **client 公開仕様** を `data/openapi.yaml` で直接表現する。現状 gateway で担われている変換層を各サービス側に内製化する。
...
「公開仕様 vs 内部仕様」の分離方針 (例: openapi に `x-internal: true` 拡張で識別、または `data/openapi.yaml` と `data/openapi-internal.yaml` のファイル分離等) は本 ADR 範囲では決めず、各サービスの判断とする。

### client 依存構成

- **各サービス TS パッケージ**: `@kenyamaneko/overload-party-api-{service}` を各サービスが新設し、client が直接消費
- **battle**: 既存 `@kenyamaneko/overload-party-game-state` を直接消費 (ADR-034 Amendment の Layer B で確定済)
...
> **更新**: gateway → 各サービス間の player_id 引き渡し方式は ADR-037 で `X-Player-Id` 平文 header から HMAC 署名 JWT (`X-Internal-Auth`) に変更された。gateway 中央認証フロー全体の方針は引き続き有効。

### Phase 3c: 各サービス TS パッケージ整備

各サービスを順次対応し、gateway openapi を徐々に縮小する。各サービスは既に `data/openapi.yaml` を持っているため、それを「公開仕様」へリファクタする形で進める (新設ではなく既存の進化)。各リポで以下を実施:
...
対象リポ: shop / card / account / scenario / news / support / matchmaking (7 リポ。末尾 Amendment で matchmaking を除外し 6 リポに改訂)
