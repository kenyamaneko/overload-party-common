...
`reactive` トリガーの過負荷と発火点の欠落を解消するため、解決モデルを次のとおり再設計する。
...
トリガー体系が event 軸に統一されて借り物トリガーが解消され、「破壊された時」「インシデント使用時」系カードが意図どおりのタイミングで発火する（サポートゾーンに居ない TK-0002 も発火経路を得る）。「いつ」と「条件」が trigger / guard に分離され、新カードはトリガー＋guard の組で表現できる。デッドコードだったチェーン機構と DB カラムも削除される。
...
`ChainResolver` / `BattleGameState.ChainStack` / `ChainEntry` / `ChainResolverTests` を削除し、DB スキーマから `chain_stack` カラムを削除するマイグレーションを行う。`chain_stack` は未使用カラムであるため、データ移行を伴わない単純な列削除で済む。
...
- `reactive` および `on_enemy_deploy` を持つカードに加え、既存 `deploy` トリガーの全カードで trigger 値の書き換えが発生する。機械的変更であり、`generate_constants.py` の unknown → throw により取りこぼしは検知される
- `on_damaged` のためにダメージ適用箇所へフックを入れる必要がある
- DB スキーマ変更（`chain_stack` カラム削除）を伴う

## 不採用案

### RULEBOOK §12 のチェーンスタックを実装する
...
### `reactive` トリガーを `reactive_on_attack` / `reactive_on_destroy` 等に細分化する
...
「連動する変更」を実装結果に合わせて修正した。カード SSoT リポ overload-party-card を追加（当初表は battle にカードデータ変更を記載していたが、トリガー・guard・effect_text の SSoT は overload-party-card の `data/cards/*.yaml`）。あわせて `docs/notes/EFFECT_YAML_SCHEMA.md` の記載を削除（同ファイルは `.gitignore` 対象のローカル専用ドキュメントで、連動する変更の追跡対象外）。
