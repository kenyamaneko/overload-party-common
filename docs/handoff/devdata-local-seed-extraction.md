...
|---|---|---|
| `data/starter_decks.yaml` | `generate_cards.py` | `devdata/cache/starter_decks_gen.json` |
| `data/news_mock.yaml` | `generate_news_mock.py` | `devdata/cache/news_mock.json` |
| `data/products.yaml` | `generate_products.py` | `devdata/cache/products_gen.json` |
...
`data/news_mock.yaml` のスキーマを拡張した場合（summary、published_at 等を追加）、
...
`data/starter_decks.yaml` を編集後、`python3 scripts/generate_cards.py` を実行すれば
