...
- **接続線:** サポートゾーンのアタッチメントカードから対象リソースへSVG破線を描画（ネットワーク図のノード間接続のイメージ）
  - 対象リソースが存在する間のみ表示
- **サポートゾーン:** プラットフォーム・リアクティブ・アタッチメントが3枠を共有。アタッチメントかどうかは接続線の有無で判別できる

**アタッチメント操作（2ステップ・順序固定）:**

1. 手札の Attachment カードをタップ → サポートゾーンの空きスロットがハイライト
2. サポートゾーンの空きスロットをタップ（配置先確定）→ スロットのハイライトが消え、選択済みスロットに青グロー表示。アタッチ可能なリソースが赤く光る
3. 光っているリソースをタップ → サーバーにアクション送信、サポートゾーンに配置＋対象リソースへの接続線が表示
4. サポートゾーン満杯 or 対象リソースの装備上限（2枚）到達時はアタッチ不可
5. 背景タップで選択解除
