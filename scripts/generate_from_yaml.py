...
This script lives in overload-party-common and generates outputs for:
  ...
  - battle:  internal/cache/cards_gen.json, internal/model/*_gen.go, internal/constants/constants_gen.go
...
def main():
    ...
    if battle_dir and battle_dir.exists():
        ...
        model_names = generate_go_models(battle_dir, "battle")
        const_path = generate_repo_constants(constants, battle_dir, "battle")
        print(f"Generated {json_count} cards → {battle_dir.name}/internal/cache/cards_gen.json", file=sys.stderr)
        print(f"Generated models → {battle_dir.name}/internal/model/ [{', '.join(model_names)}]", file=sys.stderr)
        print(f"Generated constants → {battle_dir.name}/{const_path}", file=sys.stderr)
    else:
