...
This script lives in overload-party-common and generates outputs for:
  - common: docs/CARDS.md, model/constants_gen.go, cardno/cardno_gen.go
  - gateway: internal/cache/cards_gen.json
  - battle:  internal/cache/cards_gen.json
...
def generate_go_constants(constants):
    """Generate model/constants_gen.go in common from constants.json."""
    go_out = COMMON_ROOT / "model" / "constants_gen.go"
...
def main():
    ...
    # Always generate common outputs (CARDS.md, cardno, constants)
    md_count = generate_md(cards, faction_data)
    go_count = generate_go_cardno(cards, faction_data)
    go_const_path = generate_go_constants(constants)
    ...
    print(f"Generated constants → {go_const_path}", file=sys.stderr)

    # Gateway outputs (cards_gen.json)
    ...
    # Battle outputs (cards_gen.json)
    battle_dir = Path(args.battle_dir) if args.battle_dir else None
    if battle_dir and battle_dir.exists():
        json_count = generate_json(cards, battle_dir)
        print(f"Generated {json_count} cards → {battle_dir.name}/internal/cache/cards_gen.json", file=sys.stderr)
    else:
