...
def convert_to_pascal(value):
    ...
    Args:
        value: 変換元の YAML 文字列値。

    Returns:
        PascalCase 化した識別子文字列。
    """
    value = value.replace("/", "")
...
def convert_to_screaming_snake(value):
    ...
    Returns:
        SCREAMING_SNAKE_CASE 化した識別子文字列。
    """
...
def build_go_const_block(comment, prefix, values):
    ...
    lines.append("const (")
    for v in values:
        name = f"{prefix}{convert_to_pascal(v)}"
...
def generate_go_game_design(data, factions):
    ...
    for v in data["restriction_values"]:
        lines.append(f"\tRestriction{convert_to_pascal(v)}: {copy_count[v]},")
    lines.append("}")
    lines.append("")
...
def build_cs_static_class(class_name, values, indent="    "):
...
def build_cs_header(namespace):
    """C# ファイルのヘッダーを生成します。

    Args:
        namespace: 生成する C# の namespace 名。
...
def build_ts_const_array(const_name, type_name, values, extra_union=None):
    """TypeScript の as const 配列と union type を生成します。

    Args:
        const_name: 生成する const 配列名。
        type_name: 生成する union type 名。
        values: 配列に含める値のリスト。
        extra_union: union 型に追加するリテラル (任意)。

    Returns:
        生成した TypeScript ソース行のリスト。
    """
    lines = []
