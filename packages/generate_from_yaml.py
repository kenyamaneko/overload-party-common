...
# ─── Go type → TS type mapping for WS messages ────────
...
def generate_ts_ws_messages(*, out_path):
    ...
    # Find the ws_messages file definition
    ws_file = None
    for file_def in data["files"]:
...
def main():
    ...
    ws_result = generate_ts_ws_messages(out_path=npm_dir / "src" / "wsMessages.ts")
    if ws_result:
        print(f"Generated WS messages → {rel}/npm/src/wsMessages.ts", file=sys.stderr)
