...
Usage (legacy - direct repo output):
    python3 scripts/generate_from_yaml.py [--gateway-dir PATH] [--battle-dir PATH] [--client-dir PATH]

Usage (package mode - output to gen/ for publishing):
    python3 scripts/generate_from_yaml.py --gen-dir gen/
...
# ─── Generate JSON ─────────────────────────────────────
def generate_json(cards, repo_dir=None, *, out_path=None):
    """Generate cards_gen.json.

    Args:
        repo_dir: Path to repo root (legacy mode, outputs to internal/cache/).
        out_path: Direct output file path (package mode).
    """
    if out_path is None:
...
def generate_go_models(repo_dir=None, target="gateway", *, out_dir=None):
    """Generate *_gen.go model files from models.yaml.

    Args:
        repo_dir: Path to repo root (legacy mode, outputs to internal/model/).
        target: "gateway" or "battle".
        out_dir: Direct output directory for model files (package mode).
    """
...
def generate_repo_constants(constants, repo_dir=None, target="gateway", *, out_path=None):
    """Generate constants_gen.go for a specific repo.

    Args:
        constants: parsed constants.json dict.
        repo_dir: Path to repo root (legacy mode).
        target: "gateway" or "battle".
        out_path: Direct output file path (package mode).
    """
...
def generate_ts_constants(constants, client_dir=None, *, out_path=None):
    """Generate constants.ts from constants.json.

    Args:
        client_dir: Path to client repo root (legacy mode).
        out_path: Direct output file path (package mode).
    """
    if out_path is not None:
...
def generate_csharp_constants(constants, battle_dir=None, *, out_path=None, namespace="OverloadParty.Battle.Models"):
    """Generate GameConstants_gen.cs from constants.json.

    Args:
        battle_dir: Path to battle repo root (legacy mode).
        out_path: Direct output file path (package mode).
    ...
    # In package mode (non-Battle.Models namespace), class is not partial
    is_partial = "partial " if namespace == "OverloadParty.Battle.Models" else ""
    ...
    lines = [
        ...
        "",
        f"public static {is_partial}class GameConstants",
        "{",
    ]
...
def generate_csharp_event_data(schemas, battle_dir=None, *, out_path=None, namespace="OverloadParty.Battle.Models"):
...
def generate_ts_event_data(schemas, client_dir=None, *, out_path=None):
...
def generate_md(cards, faction_data):
    ...
    lines.append("")
    lines.append(f"**全 {total} 枚** | **Last Updated:** {date.today().isoformat()}")
    lines.append("")
    lines.append("---")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--gen-dir", type=Path,
                        help="Package mode: output to gen/ directory for publishing")
    ...
    # ─── Legacy mode: output to sibling repos ─────────────
